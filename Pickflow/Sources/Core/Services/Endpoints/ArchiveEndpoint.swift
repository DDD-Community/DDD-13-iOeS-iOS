...
enum ArchiveEndpoint: APIEndpoint {
    ...
    case fetchMySpots(page: Int, latitude: Double?, longitude: Double?)
    ...
    var path: String {
        ...
        case .fetchMySpots: "/v1/users/me/my-spots"
        }
    }

    var method: HTTPMethod {
        switch self {
        case .fetchInfo, .fetchSavedSpots, .fetchMySpots: .get
    ...
    var parameters: Parameters? {
        ...
        case let .fetchSavedSpots(page, latitude, longitude),
             let .fetchMySpots(page, latitude, longitude):
