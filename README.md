...
- `release/x.y.z-hotfix` - 출시본 긴급 수정 브랜치. 절차는 일반 릴리즈와 같습니다.

```
feature → develop → release/x.y.z        → 배포 + 태그 → main 전진 → develop 백머지
           main   → release/x.y.z-hotfix → 배포 + 태그 → main 전진 → develop 백머지
```
...
- 출시 중인 버전에 급한 수정이 필요하면 `main`에서 `release/x.y.z-hotfix`를 따서 작업합니다. 자르는 출처만 다를 뿐 이후 절차는 일반 릴리즈와 같습니다.
- 핫픽스는 **Create Release** 워크플로를 쓸 수 없어 버전 상향 커밋을 직접 올립니다.
