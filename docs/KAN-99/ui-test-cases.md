...
| `kan99-sheet-content-myspot-dark` | SpotDetailSheetContentView | isMine=true, isBookmarked=false, isAddressExpanded=false | 노을 | ko_KR / .large | Dark | iPhone 15 | Heading "동작구 산책로" 옆에 주황 outline + 주황 텍스트 "MY 스팟" 뱃지. 액션 row는 "길 안내 받기" 단일 full-width 버튼만 (저장하기 버튼 없음). 그 외 영역은 collapsed-not-bookmarked와 동일 | `kan99-sheet-content-myspot-dark.png` |
